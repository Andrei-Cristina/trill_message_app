...
fun Route.userRoutes() {
    ...
    route("/login") {
        post {
            ...
            if (userId.password.isBlank() || !BCrypt.checkpw(loginRequest.password, userId.password)) {
                call.application.environment.log.warn("API_LOGIN_FAIL: Password mismatch for email [${loginRequest.email}].")
            ...
            get {
                ...
                userRepository.getByEmail(email).fold(
                    onSuccess = { user ->
                        ...
                        call.respond(HttpStatusCode.OK, user.toApiUser())
                    },
                    onFailure = { e ->
    ...
    route("/users") {
        post {
            ...
            call.application.environment.log.info(
                "Registering new user with email: {} and nickname: {}",
                userRequest.email,
                userRequest.nickname
            )

            if (userRequest.email.isBlank() || userRequest.nickname.isBlank() || userRequest.password.isBlank()) {
            ...
            userRepository.create(
                ...
                onSuccess = {
                    call.application.environment.log.info(
                        "Successfully created user with email: {}",
                        userRequest.email
                    )
