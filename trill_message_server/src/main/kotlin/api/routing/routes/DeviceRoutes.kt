    ...
    route("/devices") {
        get("/all/keys") {
            ...
            val recipientId = userRepository.getIdByEmail(recipientEmail).fold(
                onSuccess = { it },
                onFailure = { e ->
                    call.application.environment.log.warn("User not found for email: {}. Error: {}", recipientEmail, e.message)
            ...
            deviceRepository.getAllDevices(recipientId).fold(
                onSuccess = getAllDevicesFold@{ devices ->
                    ...
                    call.application.environment.log.debug("Found {} devices for user ID: {}", devices.size, recipientId)
                    ...
                    devices.forEach { device ->
                        if (device.onetimePreKeys.isNotEmpty()) {
                            ...
                            deviceRepository.deleteOneTimePreKey(device.identityKey.toString(), oneTimePreKey.toString()).fold(
                                ...
                                onFailure = { e ->
                                    call.application.environment.log.error("Failed to delete one-time pre-key for device: {}. Error: {}", device.identityKey, e.message, e)
                                    call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Failed to process device keys"))
                                    return@getAllDevicesFold
                                }
                            )
                        } else {
                            call.application.environment.log.debug("Skipping device {} with no one-time pre-keys", device.identityKey)
                        }
                    }

                    if (bundleList.isEmpty()) {
                        call.application.environment.log.warn("No devices with one-time pre-keys found for user ID: {}", recipientId)
                        call.respond(HttpStatusCode.NotFound, mapOf("error" to "No devices with one-time pre-keys found"))
                        return@getAllDevicesFold
                    }
        ...
        get("/keys") {
            ...
            deviceRepository.getOnlineDevices(recipientId).fold(
                ...
                onFailure = { e ->
                    call.application.environment.log.error("Failed to fetch online devices for user ID: {}. Error: {}", recipientId, e.message, e)
                    call.respond(HttpStatusCode.InternalServerError, mapOf("error" to "Failed to fetch devices"))
                }
            )
        }
        ...
        post {
            ...
            val userId = userRepository.getIdByEmail(registerBundle.userEmail).fold(
                onSuccess = { it },
                onFailure = { e ->
                    call.application.environment.log.warn("User not found for email: {}. Error: {}", registerBundle.userEmail, e.message)
                    call.respond(HttpStatusCode.NotFound, mapOf("error" to "User not found"))
                    return@post
                }
            )
