        ...
        route("/messages") {
            post {
                ...
                call.application.environment.log.info(
                    "Received message request with {} messages",
                ...
                for (messageWrapper in request.messages) {
                    ...
                    if (senderDevices.none { it.identityKey == message.senderDeviceId }) {
                        call.application.environment.log.warn(
                            "Sender device not found: {} for sender: {}",
                    ...
                    if (recipientDevices.none { it.identityKey == message.recipientDeviceId }) {
                        call.application.environment.log.warn(
                            "Recipient device not found: {} for recipient: {}",
                            message.recipientDeviceId,
                            message.recipientId
                        )
                        call.respond(
                            HttpStatusCode.NotFound,
                            "Recipient device not found: ${message.recipientDeviceId}"
                        )
                        return@post
                    }
            ...
            get("/{deviceId}") {
                ...
                val userId = principal.userEmail
                ...
                messageRepository.getByRecipient(userId, deviceId).fold(
                    onSuccess = { messages ->
                        call.application.environment.log.info(
                            "Returned {} messages for user: {} and device: {}",
                            messages.size,
                            userId,
                            deviceId
                        )
            ...
            get {
                call.application.environment.log.warn(
                    "Unmatched GET request to /messages: path={}",
                    call.request.path()
                )
